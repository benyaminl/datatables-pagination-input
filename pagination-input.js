 ...
 *  @author [Benyamin Limanto](https://benyamin.xyz)
 *
 *  @example
...
(function ($) {
	...
	$.fn.dataTableExt.oPagination.input = {
		'fnInit': function (oSettings, nPaging, fnCallbackDraw) {
            ...
            nInput.style.maxWidth = "100px";
			nTotal.className = paginateTotalClassName + " py-2 pr-2";
			...
			info = info.replace(/_INPUT_/g, '</li>' + nInput.outerHTML + '<li class=p-2>');
			info = info.replace(/_TOTAL_/g, '</li>' + nTotal.outerHTML + '<li>');
            nInfo.innerHTML = '<li class=p-2>' + info + '</li>';
            console.log(info);
            nContainer = $("<ul class='pagination'></ul>").get(0);
            
			nContainer.appendChild(nFirst);
            nContainer.appendChild(nPrevious);
			$(nInfo).children().each(function (i, n) {
			    nContainer.appendChild(n);
			});
			nContainer.appendChild(nNext);
            nContainer.appendChild(nLast);
